...
export default function HistoryOperation({operations, setOperations, category}){
        ...
        sortArray.map((item,index)=>{
            return <div className='operations' key={index}>
                ...
                {item.data.map((operation,index)=>{
                    ...
                    return <Operation {...operation} setOperations={setOperations} operations={operations} key={index} refText={textAreaRef} index={index} category={category}/>
                })}
            </div> 
        }):
        <p>Нет записей</p>}
